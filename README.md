...
- 平板默认使用底部训练控制台，状态、主操作和动作队列集中在视频底部，减少手指移动和视觉跳转。
- “完成本组”使用独立大按钮，训练中更容易点中，降低误触概率。
...
- 详情抽屉内通过动作、分组、录入、设置切换；默认收起，不占用训练主界面。
- 时间戳录入区保持单行动作格式，长动作名可横向滚动查看。
- 底部控制台支持收起为小标题栏，适合安卓平板、Via 等触控浏览器在 PC 模式下使用。
- 动作队列支持锁定/解锁，解锁后训练中可直接切换动作。
...
- 将平板界面完全重构为底部训练控制台，默认只显示状态、主操作和横向动作队列。
- 将动作、分组、录入、设置收进可展开详情抽屉，避免训练时被管理表单干扰。
- 兼容旧偏好配置，并把旧的“预览”tab 自动映射到新的“动作”tab。
